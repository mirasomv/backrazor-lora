...
def setup(args, log, num_classes):
	...
	# Fill LoRA config onto the model config (defaults keep LoRA off if rank=0)
	try:
	...
	config.lora.rank   = getattr(args, "lora_rank", 0)
	config.lora.alpha  = getattr(args, "lora_alpha", 1.0)
	config.lora.dropout= getattr(args, "lora_dropout", 0.0)
	config.lora.mlp    = getattr(args, "lora_mlp", False)
