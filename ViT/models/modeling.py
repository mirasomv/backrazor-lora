...
from ViT.models.configs import get_vit_s16_config
...
class LoRAAdapter(nn.Module):
    """
    LoRA(x) = (alpha / r) * dropout(x) @ A @ B
    Only A and B are trainable. Keeps the original Linear untouched.
    """
    def __init__(self, in_features: int, out_features: int, r: int, alpha: float, dropout: float = 0.0):
        super().__init__()
        assert r >= 0
        self.r = r
...
class Attention(nn.Module):
    def __init__(self, config, vis, prune_mode=False, prune_after_softmax=False, n_tokens=1):
        ...
        lora_rank = getattr(lora, "rank", getattr(config, "lora_rank", 0))
        lora_alpha = getattr(lora, "alpha", getattr(config, "lora_alpha", 1.0))
        lora_dropout = getattr(lora, "dropout", getattr(config, "lora_dropout", 0.0))

        self.lora_rank = int(lora_rank)
        self.query_lora = LoRAAdapter(config.hidden_size, self.all_head_size, self.lora_rank, lora_alpha, lora_dropout)
        self.key_lora = LoRAAdapter(config.hidden_size, self.all_head_size, self.lora_rank, lora_alpha, lora_dropout)
        self.value_lora = LoRAAdapter(config.hidden_size, self.all_head_size, self.lora_rank, lora_alpha, lora_dropout)
        # out projection is hidden_size -> hidden_size
        self.out_lora = LoRAAdapter(config.hidden_size, config.hidden_size, self.lora_rank, lora_alpha, lora_dropout)
        ...
        if self.prune_mode:
            self.attention_mask = nn.Parameter(
                torch.ones(1, self.num_attention_heads, n_tokens, n_tokens).bool(), requires_grad=False
            )
    ...
    def forward(self, hidden_states):
        ...
        mixed_key_layer = self.key(hidden_states) + self.key_lora(hidden_states)
        mixed_value_layer = self.value(hidden_states) + self.value_lora(hidden_states)
        ...
        attention_output = self.out(context_layer) + self.out_lora(context_layer)
...
class Mlp(nn.Module):
    def __init__(self, config):
        ...
        # ---- Optional LoRA for MLP (controlled by config.lora.mlp) ----
        lora = getattr(config, "lora", None)
        lora_rank = getattr(lora, "rank", getattr(config, "lora_rank", 0))
        lora_alpha = getattr(lora, "alpha", getattr(config, "lora_alpha", 1.0))
        lora_dropout = getattr(lora, "dropout", getattr(config, "lora_dropout", 0.0))
        lora_mlp = getattr(lora, "mlp", getattr(config, "lora_mlp", False))

        self.lora_rank = int(lora_rank)
        if lora_mlp and self.lora_rank > 0:
            self.fc1_lora = LoRAAdapter(config.hidden_size, config.transformer["mlp_dim"],
                                        self.lora_rank, lora_alpha, lora_dropout)
            self.fc2_lora = LoRAAdapter(config.transformer["mlp_dim"], config.hidden_size,
                                        self.lora_rank, lora_alpha, lora_dropout)
        else:
            self.fc1_lora = nn.Identity()
            self.fc2_lora = nn.Identity()
    ...
    def forward(self, x):
        # LoRA adapters take the SAME input as the base linear
        fc1_out = self.fc1(x)
        if not isinstance(self.fc1_lora, nn.Identity):
            fc1_out = fc1_out + self.fc1_lora(x)
        x = self.act_fn(fc1_out)
        x = self.dropout(x)

        fc2_out = self.fc2(x)
        if not isinstance(self.fc2_lora, nn.Identity):
            fc2_out = fc2_out + self.fc2_lora(x)
